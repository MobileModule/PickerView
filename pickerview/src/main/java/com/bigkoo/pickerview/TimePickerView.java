...
public class TimePickerView extends BasePickerView implements View.OnClickListener {
    ...
    public static class Builder {
        ...
        public Builder setType(Type type) {
            this.type = getTypeParse(type);
            return this;
        }

        private boolean[] getTypeParse(Type type) {
            boolean[] typeBool = new boolean[6];
            if (type == Type.ALL) {
                typeBool = new boolean[]{true, true, true, true, true, true};
            }
            if (type == Type.YEAR_MONTH_DAY) {
                typeBool = new boolean[]{true, true, true, false, false, false};
            }
            if (type == Type.HOURS_MINS) {
                typeBool = new boolean[]{false, false, false, true, true, false};
            }
            if (type == Type.MONTH_DAY_HOUR_MIN) {
                typeBool = new boolean[]{false, true, true, true, true, false};
            }
            if (type == Type.YEAR_MONTH) {
                typeBool = new boolean[]{true, true, false, false, false, false};
            }
            if (type == Type.YEAR_MONTH) {
                typeBool = new boolean[]{true, true, false, false, false, false};
            }
            if (type == Type.YEAR) {
                typeBool = new boolean[]{true, false, false, false, false, false};
            }
            return typeBool;
        }
